...
class dA(object):
    ...
    def __init__(
        ...
        self.params_da = [self.W, self.b, self.b_prime]
        self.params_nc = [self.W1, self.b1, self.W2, self.b2]
    ...
    def get_cost_updates(self, corruption_level, learning_rate):
        ...
        # to its parameters
        gparams = T.grad(cost_da, self.params_da)
        ...
        loss_nc = T.nnet.categorical_crossentropy(y_hat, self.lable).mean() + loss_reg

        # Returns a class prediction
        prediction = T.argmax(y_hat, axis=1)

        # Easy: Let Theano calculate the derivatives for us!
        dW2 = T.grad(loss_nc, self.W2)
        db2 = T.grad(loss_nc, self.b2)
        dW1 = T.grad(loss_nc, self.W1)
        db1 = T.grad(loss_nc, self.b1)

        # Theano functions that can be called from our Python code
        forward_prop = theano.function([y], y_hat)
        calculate_loss = theano.function([y, self.lable], loss_nc)
        predict = theano.function([y], prediction)

        # generate the list of updates
        '''
        updates = numpy.concatenate((
            [(param, param - learning_rate * gparam)
             for param, gparam in zip(self.params_da, gparams)],
             [(self.W2, self.W2 - self.epsilon * dW2),
              (self.W1, self.W1 - self.epsilon * dW1),
              (self.b2, self.b2 - self.epsilon * db2),
              (self.b1, self.b1 - self.epsilon * db1)]), axis = 0)
        '''
        ...
        return (cost_da, loss_nc, updates)
...
def test_dA(learning_rate=0.1, training_epochs=8,
            letter = 'A',
            batch_size=500, output_folder='dA_plots', hiddensize = 500, draw_pic_switch = True, noise = 'zero'):
    ...
    dataset='mnist.pkl.gz'  # We are not using this dataset
    ...
    da = dA(
        ...
        nn_hdim = 700,
        nn_output_dim = 10,
        reg_lambda = numpy.float32(0.0001), # regularization strength
        epsilon = numpy.float32(0.01) # learning rate for gradient descent
    )

    cost_da, loss_nc, updates = da.get_cost_updates(
        corruption_level=0.,
    ...
    for epoch in range(training_epochs):
        ...
        for batch_index in range(n_train_batches):
            l_da, l_nc = train_da(batch_index)
            list_da.append(l_da)
            list_nc.append(l_nc)

        print('Training epoch %d, cost ' % epoch, numpy.mean(list_da))
        print('Neural network classifier %d, cost ' % epoch, numpy.mean(list_nc))
    ...
    p = da.predict(train_set_x).eval()

    print(p[0:100])

    classes = numpy.argmax(train_set_y.eval(), axis=1)
    ...
    for i in range(classes.shape[0]):
        if p[i] != classes[i]:
            error += 1
    print('********error percent :')
    print(error / classes.shape[0])

    print('**********prediction on valid set: ')
    valid_set_x, valid_set_y = datasets[1]

    p = da.predict(valid_set_x).eval()

    print(p[0:100])

    classes = numpy.argmax(valid_set_y.eval(), axis=1)
    ...
    p = da.predict(test_set_x).eval()

    print(p[0:100])

    classes = numpy.argmax(test_set_y.eval(), axis=1)
    ...
    if(draw_pic_switch == True):
        ...
        image = Image.fromarray(
            tile_raster_images(X=da.W.get_value(borrow=True).T * 500,
